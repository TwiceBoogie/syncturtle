package com.syncturtle.services.instance.services.setup;
...
public class InstanceRegistrar {
    ...
    public void run(String requestedMachineSignature) {
        Instance instance = repo.findFirstByDeletedAtIsNullOrderByCreatedAtDesc().orElse(null);
        ...
        InstanceEdition edition = InstanceEditions.parseEdition(props.getEdition());
        String instanceName = InstanceEditions.defaultInstanceName(edition);

        String vmHost = Strings.firstNonBlank(
                System.getenv("VM_HOST"),
                System.getenv("HOSTNAME")).orElse(null);

        String namespace = Strings.firstNonBlank(

...
public class UserAuthRuntimeCacheConfiguration {
    ...
    private static final String CONTRACT_VERSION = "v3";

    @Bean
    RedisCacheManagerBuilderCustomizer redisCacheManagerBuilderCustomizer(RedisKeyBuilder redisKeyBuilder,
            @Value("${app.cache.runtime-cache.ttl}") Duration ttl) {
        RedisSerializer<UserAuthRuntimeSnapshot> serializer = valueSerializer();
    ...
    static RedisSerializer<UserAuthRuntimeSnapshot> valueSerializer() {
        return new JacksonJsonRedisSerializer<>(UserAuthRuntimeSnapshot.class);
    }

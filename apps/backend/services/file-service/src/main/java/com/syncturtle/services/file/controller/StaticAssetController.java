...
public class StaticAssetController {
    ...
    public ResponseEntity<Void> redirectToStaticAsset(@CurrentUser UUID currentUserId, @PathVariable UUID assetId) {
        StaticAssetUrlResponse response = service.signedStaticAssetUrl(currentUserId, assetId);

        return ResponseEntity.status(HttpStatus.FOUND)
                .cacheControl(CacheControl.noStore().cachePrivate())
                .header(HttpHeaders.LOCATION, URI.create(response.getSignedUrl()).toString())
                .build();
    }

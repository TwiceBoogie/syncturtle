package com.syncturtle.services.file.service.collaborator.asset;
...
public class FileAssetPolicy {
    ...
    public void requireCreateAllowed(FileAssetPurpose purpose, UUID workspaceId, String contentType, long sizeBytes) {
        ...
        boolean userAsset = purpose == FileAssetPurpose.USER_AVATAR || purpose == FileAssetPurpose.USER_COVER;
        if (userAsset && workspaceId != null) {
        ...
        long maximumSizeBytes = uploadProperties.getMaxImageSize().toBytes();
        if (sizeBytes > maximumSizeBytes) {
            throw FileException.assetTooLarge(sizeBytes, maximumSizeBytes);
        }
    ...
    public void requireStorageObjectMatchesDeclaration(FileAsset asset, Map<String, Object> metadata,
        ...
        String normalizedDeclaredType = normalizeImageContentType(asset.getContentType());
        String normalizedActualType = normalizeImageContentType(actualContentType);
        if (!Objects.equals(normalizedActualType, normalizedDeclaredType)) {
            throw FileException.assetStorageMetadataInvalid(asset.getId());
        }

        Object versionId = metadata.get("VersionId");
        if (!(versionId instanceof String value) || !StringUtils.hasText(value)) {
            throw FileException.assetStorageMetadataInvalid(asset.getId());
        }

        String detectedContentType = detectImageContentType(objectPrefix);
        if (!Objects.equals(detectedContentType, normalizedDeclaredType)) {
            throw FileException.assetStorageMetadataInvalid(asset.getId());
        }
    }
    ...
    // https://stackoverflow.com/questions/26426245/is-it-possible-to-find-an-image-file-extension-from-its-byte-array-contents

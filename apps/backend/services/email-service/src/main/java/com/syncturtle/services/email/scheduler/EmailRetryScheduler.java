...
@ConditionalOnProperty(prefix = "app.email.inbox.scheduler", name = "enabled", havingValue = "true", matchIfMissing = true)
public class EmailRetryScheduler {

    private final EmailInboxService service;

    @Scheduled(fixedDelayString = "${app.email.inbox.scheduler.poll-delay:10s}")

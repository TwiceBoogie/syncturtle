...
import com.syncturtle.platform.gateway.support.SessionId;
...
public class SessionIssueGatewayFilterFactory
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> chain.filter(exchange).then(Mono.defer(() -> {
            ...
            String rawUserId = response.getHeaders().getFirst(GatewayHeaders.HDR_INTERNAL_USER_ID);
            String rawType = response.getHeaders().getFirst(GatewayHeaders.HDR_INTERNAL_SESSION_TYPE);
            String rolesCsv = response.getHeaders().getFirst(GatewayHeaders.HDR_INTERNAL_ROLES);
    ...
    private static void stripInternalHeaders(ServerHttpResponse response) {
        response.getHeaders().remove(GatewayHeaders.HDR_INTERNAL_USER_ID);
        response.getHeaders().remove(GatewayHeaders.HDR_INTERNAL_SESSION_TYPE);
        response.getHeaders().remove(GatewayHeaders.HDR_INTERNAL_ROLES);
    }

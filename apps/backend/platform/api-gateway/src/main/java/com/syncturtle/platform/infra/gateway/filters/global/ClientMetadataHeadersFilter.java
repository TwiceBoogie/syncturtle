...
public class ClientMetadataHeadersFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        ServerHttpRequest mutated = request.mutate()
                .headers(h -> {
                    h.remove(GatewayHeaderNames.HDR_CLIENT_IP);
                    h.remove(GatewayHeaderNames.HDR_CLIENT_UA);

                    if (clientIp != null && !clientIp.isBlank()) {
                        h.set(GatewayHeaderNames.HDR_CLIENT_IP, clientIp);
                    }
                    if (ua != null && !ua.isBlank()) {
                        h.set(GatewayHeaderNames.HDR_CLIENT_UA, ua);
                    }
                }).build();

    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        return chain.filter(exchange)
                .doFinally(signalType -> {
                    ...
                    // log.info("target={} cbEx={} cbMsg={}", target, (cbEx != null ?
                    // cbEx.getClass().getName() : "-"),
                    ...
                    String ua = reqHeaders.getFirst(GatewayHeaders.HDR_CLIENT_UA);
                    String ip = reqHeaders.getFirst(GatewayHeaders.HDR_CLIENT_IP);

                    String userId = reqHeaders.getFirst(GatewayHeaders.HDR_AUTH_USER_ID);
                    String workspaceId = reqHeaders.getFirst(GatewayHeaders.HDR_AUTH_WORKSPACE_ID);

                    String corrId = firstNonBlank(
                            response.getHeaders().getFirst(GatewayHeaders.HDR_CORRELATION_ID),
                            reqHeaders.getFirst(GatewayHeaders.HDR_CORRELATION_ID));
                    String reqId = firstNonBlank(
                            response.getHeaders().getFirst(GatewayHeaders.HDR_REQUEST_ID),
                            reqHeaders.getFirst(GatewayHeaders.HDR_REQUEST_ID));

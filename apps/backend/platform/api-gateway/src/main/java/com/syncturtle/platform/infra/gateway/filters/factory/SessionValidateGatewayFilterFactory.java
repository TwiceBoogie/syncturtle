            ...
            return sessions.find(config.getSessionType(), sessionId)
                    ...
                    .flatMap(sess -> {
                        ...
                        ServerHttpRequest mutated = exchange.getRequest().mutate()
                                .headers(h -> {
                                    h.set(GatewayHeaderNames.HDR_AUTH_USER_ID, sess.getUserId().toString());
                                    h.set(GatewayHeaderNames.HDR_AUTH_SESSION_TYPE, sess.getSessionType().name());
                                    h.set(GatewayHeaderNames.HDR_AUTH_SESSION_ID, sessionId);
                                })
                                .build();

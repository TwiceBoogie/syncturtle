...
import type { IFileUploadResult } from "@syncturtle/types";
...
const fileService = new AssetOrchestratorService();
...
export const UserImageUploadModal: FC<IUserImageUploadModalProps> = (props) => {
  ...
  const handleSubmit = async () => {
    ...
    const promise = fileService.uploadUserAvatar(image).then((data: IFileUploadResult) => {
  ...
  const handleImageRemove = async () => {
    ...
    const promise = fileService.deleteAsset(value);
    toast.promise(promise, {
  ...
  const imageSrc = useMemo(() => {
    ...
    if (value && value.trim()) return getFileURL(value);

...
export class WorkspaceStore extends ExternalStore<TWorkspaceSnapshot> implements IWorkspaceStoreInternal {
  ...
  private workspaceListCache = { version: -1, list: [] as IWorkspace[] };
  ...
  get workspacesCreatedByCurrentUser(): IWorkspace[] | null {
    ...
    const userWorkspaces = this.workspaceList.filter((workspace) => workspace.createdById === userId);
    return userWorkspaces;
  }

  // computed actions
  public getWorkspaceRedirectionUrl = () => {
  ...
  public getWorkspaceBySlug = (workspaceSlug: string): IWorkspace | null => {
    ...
    return this.workspaceList.find((workspace) => workspace.slug === workspaceSlug) ?? null;
  };
  ...
  public updateWorkspaceLogo = (workspaceSlug: string, logoURL: string): void => {
    ...
    set(nextWorkspace, ["logoUrl"], logoURL);
  ...
  // computed helpers
  private get workspaceIds(): string[] {
  ...
  private upsertOne(workspace: IWorkspace): void {
    ...
    set(next, [workspace.id], workspace);

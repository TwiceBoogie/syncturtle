import type { IUserLite, TMemberOrderByOptions, TUserPermissions } from "@syncturtle/types";
...
export const getMemberSortKey = (memberDetails: IUserLite, field: string, memberRole?: string): string | Date => {
  switch (field) {
    case "display_name":
      return memberDetails.displayName?.toLowerCase() || "";
    case "full_name": {
...
export const filterWorkspaceMembersByRole = <T extends { role: string | TUserPermissions; isActive?: boolean }>(
  members: T[],
  roleFilters: string[]
): T[] => {
...
export const sortWorkspaceMembers = <T extends { role: TUserPermissions; isActive?: boolean }>(
  members: T[],
  memberDetailsMap: Record<string, IUserLite>,
  getMemberKey: (member: T) => string,
  ...
  if (!filters?.orderBy) return filteredMembers;
